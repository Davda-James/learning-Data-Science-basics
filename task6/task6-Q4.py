...
def plot_me(y,samples):
    ...
    for i in range(1,4):
        row = 0 if i < 2 else 1
        col = i % 2
        axs[row,col].hist(samples[i],bins=15, ec="black" ,label=f'Sample {i+1}',color=color[i-1])
...
def conclusion(samples):
    ...
    return means,var
samples=[]
rate=float(input("Enter lambda value : "))
n=50
yvalues=np.random.exponential(1/rate,size=1000)
...
for i in range(5):
    ...
    print(f"variance of sample{i+1} is : {var[i]}")
...
def plot_all(new_samples):
    ...
    for i in range(4):
        row = 0 if i < 2 else 1
        col = i % 2
        axs[row,col].hist((new_samples[i]),bins=15,ec="black" ,label=f'Sample {i+1}',color=color[i-1])
        axs[row,col].set_title('Random Samples')    
        axs[row,col].set_xlabel("x values")
        axs[row,col].set_ylabel("probability density")
    plt.tight_layout()
...
new_samples=[]
for i in range(4):
    data=np.random.choice(yvalues,size=m+10*i,replace=True)
